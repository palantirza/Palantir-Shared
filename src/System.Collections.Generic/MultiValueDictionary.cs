...
namespace System.Collections.Generic
{
		...
		IReadOnlyDictionary<TKey, IReadOnlyCollection<TValue>>
	{
        ...
        ** Constructors
		...
		{
			if (capacity < 0)
				throw new ArgumentOutOfRangeException("capacity", Resource.ArgumentOutOfRange_NeedNonNegNum);
        ...
        ** Static Factories
			...
			where TValueCollection : ICollection<TValue>, new()
		{
			if (new TValueCollection().IsReadOnly)
				throw new InvalidOperationException(Resource.Create_TValueCollectionReadOnly);
				...
				get
				{
					switch (state)
					{
						case EnumerationState.BeforeFirst:
							throw new InvalidOperationException((Resource.InvalidOperation_EnumNotStarted));
						case EnumerationState.AfterLast:
							throw new InvalidOperationException((Resource.InvalidOperation_EnumEnded));
						default:
							return current;
					}
				}
			...
			IReadOnlyCollection<TValue>
		{
            ...
            ** Shared API
			...
			public void CopyTo(TValue[] array, int arrayIndex)
			{
				...
				if (arrayIndex < 0)
					throw new ArgumentOutOfRangeException("arrayIndex", Resource.ArgumentOutOfRange_NeedNonNegNum);
				if (arrayIndex > array.Length)
					throw new ArgumentOutOfRangeException("arrayIndex", Resource.ArgumentOutOfRange_Index);
				if (array.Length - arrayIndex < collection.Count)
					throw new ArgumentException(Resource.CopyTo_ArgumentsTooSmall, "arrayIndex");
            ...
            ** Public-Facing API
			...
			{
				throw new NotSupportedException(Resource.ReadOnly_Modification);
			}
